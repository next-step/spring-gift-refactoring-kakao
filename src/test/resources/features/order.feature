...
Feature: 주문 관리
  ...
  @state
  Scenario: OR-W1 주문 후 해당 상품의 위시가 정리된다
    Given 포인트가 충분한 회원이 존재하고 유효한 토큰을 가진다
    And "교환권" 카테고리에 가격 5000인 "아메리카노" 상품이 존재한다
    And 해당 상품에 수량 100인 "기본옵션" 옵션이 존재한다
    ...
    Then 응답 상태 코드는 200
    And 응답 body의 "page.totalElements"가 0이다

  @state
  Scenario: OR-W2 주문 후 다른 상품의 위시는 유지된다
    Given 포인트가 충분한 회원이 존재하고 유효한 토큰을 가진다
    And "교환권" 카테고리에 가격 5000인 "아메리카노" 상품이 존재한다
    And 해당 상품에 수량 100인 "기본옵션" 옵션이 존재한다
    And "교환권" 카테고리에 "라떼" 상품이 존재한다
    And 해당 회원에게 "라떼" 상품 위시가 이미 등록되어 있다
    When 인증된 사용자가 수량 1으로 주문 생성 요청을 보낸다
    Then 응답 상태 코드는 201
    When 인증된 사용자가 위시 목록을 페이지 0 사이즈 10로 조회 요청을 보낸다
    Then 응답 상태 코드는 200
    And 응답 body의 "page.totalElements"가 1이다
