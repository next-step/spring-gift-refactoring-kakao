...
public class ProductSteps {
  ...
  public void 상품_목록을_조회하면_포함되어_있다(String productName) {
    ExtractableResponse<Response> listResponse =
        RestAssured.given().log().all().when().get("/api/products").then().log().all().extract();
  ...
  public void 상품_목록에_포함되어_있다(String name1, String name2) {
    ...
    assertThat(response.jsonPath().getList("content.name", String.class)).contains(name1, name2);
  }
}
