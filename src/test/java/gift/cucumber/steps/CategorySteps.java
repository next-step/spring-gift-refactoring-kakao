...
public class CategorySteps {
  ...
  public void 카테고리를_생성한다(String name) {
    ExtractableResponse<Response> response =
        RestAssured.given()
            ...
            .body(
                Map.of("name", name, "color", "#000000", "imageUrl", "http://img.com/default.png"))
            .when()
            .post("/api/categories")
            .then()
            .log()
    ...
    if (state.getCategoryId(name) == null) {
      ExtractableResponse<Response> response =
          RestAssured.given()
              ...
              .body(
                  Map.of(
                      "name", name, "color", "#000000", "imageUrl", "http://img.com/default.png"))
              .when()
              .post("/api/categories")
              .then()
              .log()
  ...
  public void 카테고리_목록에_포함되어_있다(String name1, String name2) {
    ...
    assertThat(response.jsonPath().getList("name", String.class)).contains(name1, name2);
  }
}
