...
4. **구조 변경과 작동 변경의 분리** — 하나의 작업에서 구조 변경과 작동 변경을 동시에 수행하지 않는다. 사용자가 지시한 종류의 변경만 수행한다.
5. **하나의 작업 단위** — 사용자가 요청하면 반드시 하나의 작업만 수행한다. 사용자가 지시하지 않은 작업은 절대 동시에 진행하지 않는다.
...
변경 종류에 따른 검증 방법을 기술한다.

- **구조 변경의 경우**: 기존 테스트가 모두 통과하는지 확인 (입력/출력 보존)
- **작동 변경의 경우**: 변경된 동작을 검증하는 테스트를 명시하고, 의도하지 않은 부수 효과가 없는지 확인
- 실행할 테스트
- 확인할 항목
```
...
### 1단계: 현황 분석 및 변경 종류 식별

리팩터링 대상의 현재 상태를 파악하고, 요청된 변경의 종류를 식별한다.

```
1. 대상 파일/패키지 읽기
2. 의존성, 책임, 구조 파악
3. 문제점 식별 및 근거 수집
4. 변경 종류 식별: 구조 변경인지 작동 변경인지 판단
5. 구조 변경과 작동 변경이 섞여 있다면, 분리하여 사용자에게 확인
```
