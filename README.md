...
TDD의 "테스트 먼저" 원칙을 따라 0단계에서 모든 테스트를 선행 작성하려 했으나, 컴파일 에러를 일으키는 테스트(존재하지 않는 메서드 호출)는 전체 빌드를 깨뜨렸다. `OptionTest.calculateTotalPrice`는 `Option`에 메서드가 없어 컴파일 자체가 불가능했고, 4단계에서 메서드 구현과 함께 작성했다. 컴파일 언어에서 Red 테스트 선행 작성은 "실행 시 실패"와 "컴파일 에러"를 구분해야 한다. 리뷰에서 빈 시그니처(`return 0;`)를 먼저 선언해두고 Red 테스트를 작성한 뒤 구현하는 방식을 제안받았는데, 이렇게 하면 컴파일 에러 없이 TDD 흐름을 유지할 수 있다. 다만 여러 단계의 테스트를 한꺼번에 선행 작성하면 빈 시그니처가 코드베이스에 흩어지므로, 한 단계씩 빈 시그니처 → Red → Green을 반복하는 것이 적합하다.
...
#### 트랜잭션 경계에서 외부 호출을 분리해야 한다

`@Transactional` 안에서 카카오 API를 호출하고 있었는데, 리뷰에서 외부 API 응답이 느려지면 DB 커넥션을 그만큼 오래 점유한다는 지적을 받았다. `TransactionTemplate`으로 전환하여 DB 작업만 트랜잭션으로 감싸고, 외부 호출은 커밋 후에 실행하도록 분리했다. 트랜잭션 경계를 설정할 때 "이 안에 외부 I/O가 포함되어 있는가"를 반드시 확인해야 한다.

#### 금액 계산에서 int 오버플로우는 조용히 발생한다

`calculateTotalPrice`가 `int`를 반환하고 있었는데, 리뷰에서 100,000원 × 30,000개 = 3,000,000,000이 `int` 최대값(2,147,483,647)을 초과한다는 지적을 받았다. int 오버플로우는 예외 없이 음수로 감싸지므로 테스트 없이는 발견하기 어렵다. 금액처럼 곱셈으로 큰 수가 나올 수 있는 계산은 `long`을 사용하고, 경계값 테스트를 작성해두는 것이 안전하다.
