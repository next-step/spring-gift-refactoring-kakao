...
리팩터링 완료 후 전체 코드를 검토하면서, 기존 64개 시나리오 대부분이 "요청 → 상태코드 확인"으로 끝나는 계약 테스트라 실제 데이터 변경이 올바른지 검증하지 못하는 한계를 발견했다. 이를 보완하기 위해 **상태 변화 검증 시나리오 11개**를 서비스 추출 이후에 추가했다. 안전망 역할이라면 리팩터링 이전에 작성했어야 하므로, 타이밍상 안전망의 역할을 온전히 한 것은 아니다:
...
리팩터링 과정이 이번 과제의 핵심이라 판단해, AI는 테스트 구축에 집중적으로 활용하고 본 코드의 리팩터링(서비스 추출, 패키지 재편, 아키텍처 설계)은 직접 수행했다.
...
| 서비스 추출  | —                                  | 설계, 구현, 계층 경계 확정 (직접 수행)                |
| 아키텍처 설계 | —                                  | internal 패키지, Port 패턴, 전략 패턴 결정 (직접 수행) |
