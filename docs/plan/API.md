...
| `/api/products?page=0&size=10&sort=name,asc&categoryId=1` | GET | 상품 목록 조회 | 모든 상품의 목록을 페이지 단위로 조회한다. |
...
| `/api/wishes?page=0&size=10&sort=createdDate,desc` | GET | 위시 리스트 상품 조회 | 회원의 위시 리스트에 있는 상품을 페이지 단위로 조회한다. |
...
| `/api/orders?page=0&size=10&sort=orderDateTime,desc` | GET | 주문 목록 조회 | 주문 목록을 페이지 단위로 조회한다. |
